#!/bin/bash
# ===========================
# RSA TCP Demo Launcher - Linux/Unix
# ===========================
...
echo -e "${CYAN}================================================${NC}"
echo -e "${CYAN}   RSA over TCP - Demo Launcher (Linux)       ${NC}"
echo -e "${CYAN}================================================${NC}"
echo ""
...
if command -v gnome-terminal &> /dev/null; then
    ...
    echo -e "${YELLOW}Supported terminals: gnome-terminal, xfce4-terminal, konsole, xterm${NC}"
    exit 1
fi
...
# Launch Alice based on terminal type
case "$TERMINAL" in
    gnome-terminal)
        gnome-terminal -- bash -c "cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$ALICE_SCRIPT'; exec bash"
        ;;
    xfce4-terminal)
        xfce4-terminal --hold -e "bash -c \"cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$ALICE_SCRIPT'\""
        ;;
    konsole)
        konsole --hold -e bash -c "cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$ALICE_SCRIPT'"
        ;;
    xterm)
        xterm -hold -e "cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$ALICE_SCRIPT'" &
        ;;
esac
...
# Launch Bob based on terminal type
case "$TERMINAL" in
    gnome-terminal)
        gnome-terminal -- bash -c "cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$BOB_SCRIPT'; exec bash"
        ;;
    xfce4-terminal)
        xfce4-terminal --hold -e "bash -c \"cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$BOB_SCRIPT'\""
        ;;
    konsole)
        konsole --hold -e bash -c "cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$BOB_SCRIPT'"
        ;;
    xterm)
        xterm -hold -e "cd '$PROJECT_ROOT' && '$PYTHON_EXE' '$BOB_SCRIPT'" &
        ;;
esac
